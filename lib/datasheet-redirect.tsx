          ...
          <main className="pt-20 md:pt-28 lg:pt-32 pb-12 md:pb-16">
            <div className="mx-auto w-full max-w-[1100px] px-3 sm:px-4 md:px-8">
              <header className="mb-8 border-b border-[#2a2a36] pb-6">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-balance leading-tight">{product.name}</h1>
                {product.description && (
                  <p className="mt-3 text-base md:text-lg text-[#cfd0da] text-pretty">{product.description}</p>
                )}
              </header>

              <GoogleDrivePdfViewer shareLink={product.datasheet} title={`${product.name} datasheet`} />

              <a
                href={`/products/${brandId}`}
                className="mt-10 inline-flex items-center gap-2 rounded-lg border border-[#f5b800]/30 px-5 py-3 text-sm text-[#f0f0f5] hover:border-[#f5b800] transition-colors"
              >
